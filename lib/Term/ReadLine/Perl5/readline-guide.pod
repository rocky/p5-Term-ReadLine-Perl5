...
A non-OO package similar to GNU's readline. The prefered OO Package is
L<Term::ReadLine::Perl5>. But that uses this internally.

There is some support for EUC-encoded Japanese text. This should be
rewritten for Perl Unicode though.

Originally author: Jeffrey Friedl, Omron Corporation
(jfriedl@omron.co.jp)
